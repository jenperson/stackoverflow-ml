...
let addTabs = (response) => {
  ...
  let top5 = getTop5(classifications)
  for (let label of top5) {
    var selector = {label: label};
      ...
      chrome.tabs.executeScript({
        code: '(' + function(params) {
          document.querySelector('#tagnames').value += params.label + " "
          console.log("like yeah", params.label);
            return {success: true, html: document.body.innerHTML};
        } + ')(' + JSON.stringify(selector) + ');'
      });
    }
